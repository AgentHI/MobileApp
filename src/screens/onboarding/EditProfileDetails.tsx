...
export const EditProfileDetails = () => {
  ...
  const onSaveAndContinue = async () => {
    ...
    if (get(agentCreate, '_id', false)) {
      ...
      let agentData = await FetchAgentViaEmail(email);
      const userData = await AsyncStorage.getItem('@user_data') ?? '';
      const jsonValue = JSON.parse(userData);
      console.log('jsonValue__', agentData);
      
      await AsyncStorage.setItem('@user_data', JSON.stringify({...jsonValue, agent: agentData.agent, user: agentData.agent}));
      navigation.goBack();
    }
    setLoading(false);
  };
              ...
              {userPic ? (
                <TouchableOpacity onPress={openImagePicker}>
                  <View style={styles.userPicView}>
                    <FastImage
                      source={{uri: userPic.path ? userPic.path : userPic}}
